...
public class RandomUtils {
    ...
    public static String number(Long id) {
        if (id == null) {
            id = randomNumber();
        }
    ...
    private static Long randomNumber() {
        return Double.valueOf((Math.random() * 9 + 1) * 1000).longValue();
    }
    ...
    public static String name(int len) {
        ...
        String[] particleList = {
          "の", "之", "者", "所", "然", "夫", "乎", "焉", "等", "且", "也", "矣", "耳", "而已", "云", "尔"
        };
        int surnameSize = surnameList.length;
        int nameSize = nameList.length;
        StringBuilder sb = new StringBuilder();
        Random random = new Random();
        sb.append(surnameList[random.nextInt(surnameSize)]);
        sb.append(nameList[random.nextInt(surnameSize)]);
        sb.append(particleList[random.nextInt(particleList.length)]);
        int surLen = sb.toString().length();
        for (int i = 0; i < len - surLen; i++) {
            if(sb.toString().length() <= len){
                sb.append(nameList[random.nextInt(nameSize)]);
            }
        }
        return sb.toString();
    }
    ...
    public static void main(String[] args) {
        for (int i = 0; i < 10; i++) {
            System.out.println(RandomUtils.name(5));
        }
    }
}
